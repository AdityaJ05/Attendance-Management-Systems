...
#connecting it with the database name="attendance"
con=sql.connect(host='127.0.0.1', user='root', password='', database='attendance')
cur=con.cursor(buffered=True)
def show_frame(frame):
    frame.tkraise()                #helps in switching windows in tkinter
...
#gets the list of all students names in faces dir
...
#encodes the paticular student along with its correct name from the above list
...
#while marking the attendance, these fields will be updated
def attendance(name,time,storein):
    cur.execute("UPDATE "+storein+" SET attendance='Present' WHERE time='"+time+"' AND regid='"+name+"'")
    con.commit()


#the encoded list will show the name of the recognized face of student
encodeListKnown = faceEncodings(images)
print('All Encodings Complete!!!')
master = Tk()
master.title('Attendance System')     #creating window 
...
#frame1(attendance marking page)
...
cur.execute("SELECT cname FROM classes")         #it will show all the available classes mentioned in database
...
cur.execute("SELECT tname FROM teachers")        #it will show all the available teachers mentioned in database
...
cur.execute("SELECT sub FROM subjects")        #it will show all the available subjects mentioned in database
subs=cur.fetchall()
subsl=[]
for i in subs:
...
cur.execute("SELECT dname FROM storein")       #it will show all the available divisions along with month mentioned in database
...
def sub():
    ...
    #this will mark the attendance of student and will fetch the students
    con.commit()
    ...
    #this will update all the fields as mentioned bt the student
    con.commit()
    while True:
        ret, frame = cap.read()
        ...
        #this will compare the recognized face with the encoded list if there is a match 
        for encodeFace, faceLoc in zip(encodesCurrentFrame, facesCurrentFrame):
            matches = face_recognition.compare_faces(encodeListKnown, encodeFace)
            faceDis = face_recognition.face_distance(encodeListKnown, encodeFace)
            matchIndex = np.argmin(faceDis)

            #if the face matches then the corresponding name will be displayed and attendance will be marked
        ...
        #this is for webcam if the face is not recognized then frame5 will be shown
...
def addDefaulter():
    defaulter=View()
    f = open('C:\Aditya\Face Recognition\defaulters.csv', 'w', newline='')
    writer = csv.writer(f)
...
#fetching the attendance record
def View():
    defaulter=[]       
...
#frame4(record wise- paticular student's records)
